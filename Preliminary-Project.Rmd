---
title: "Growth-Promoting Fungi Statistical Re-evaluation"
...
2.  Carry out a Principal Component Analysis (PCA) and a Linear Discriminant Analysis (LDA) to characterize the data obtained from the greenhouse experiment. 

3.Evaluate the performance of the strains selected by the authors by doing statistical analysis using the data provided. 
...
Different organized data frames where created from the raw data. These data frames contain the different strains tested , the repetitions and the biochemical or growth parameters. 
...
```
 Lillie test Was chosen to do the in order to test the normality because it allows the processing of more than 50 dependent variables at once. 
...
````{r IAA K-W}
D_IAA$names <- paste0("LSM_", D_IAA$LSM_strain ) #Add column with strain name

kr <- agricolae::kruskal(D_IAA[2], D_IAA[3], alpha = 0.05, p.adj ="holm", group=TRUE)
```

```{r K-W plot,  out.width= "170%", fig.cap = "Graph 1: Indolacetic acid concentrations Holms groups (microgram per gram of dried micelium)"}
cp <- colorRampPalette(c ("navy", "lightskyblue", "orange", "maroon1")) #Color palette
dev.new (width=17, height= 5)
plot.new()
plot(kr, main = "Indolacetic acid produced by each strain (Non Parametric Analysis)", xlab = "Strain ID", ylab = "Indolacetic acid produced (microg/g)", col = cp (9), las = 2, cex = 0.8, cex.axis=0.8, cex.names=0.6)
...
n_D_IAA$names <- paste0("LSM_", n_D_IAA$D_IAA.LSM_strain ) #Add column with strain name
ANOVA_n_IAA<- aov (n_D_IAA$n_IAA ~ n_D_IAA$names, data= n_D_IAA)
...
T_nIAA <- HSD.test(ANOVA_n_IAA,'n_D_IAA$names', group = TRUE, alpha = 0.05)

cp1 <- colorRampPalette(c ("maroon1", "orange")) #Color palette
dev.new (width=17, height= 5)
plot.new()
plot(T_nIAA, main = "Indolacetic acid produced by each strain (Tukey)", xlab = "Strain", ylab = "Indolacetic acid produced (microg/g)", col = cp1(3), las = 2, cex = 0.8, cex.axis=0.8, cex.names=0.6)


```
Also, Duncan test was done, in order to learn if there is some differences with Tukey's test:
```{r IAA Duncan, out.width= "170%"}
D_nIAA <- duncan.test(ANOVA_n_IAA,'n_D_IAA$names')

cp1 <- colorRampPalette(c ("maroon1", "orange")) #Color palette
dev.new (width=17, height= 5)
plot.new()
plot(D_nIAA, main = "Indolacetic acid produced by each strain (Duncan)", xlab = "Strain", ylab = "Indolacetic acid produced (microg/g)", col = cp1(13), las = 2, cex = 0.8, cex.axis=0.7, cex.names=0.6)
...
````{r P normality test}
D_Phosphorus$names <- paste0("LSM_", D_Phosphorus$LSM_strain ) #Add column with strain name

ANOVA_P<- aov (D_Phosphorus$P_pg ~ D_Phosphorus$names, data= D_Phosphorus)
...
T_P <- HSD.test(ANOVA_P,'D_Phosphorus$names', group = TRUE, alpha = 0.05)

cp1 <- colorRampPalette(c ("maroon1", "orange")) #Color palette
dev.new (width=17, height= 5)
plot.new()
plot(T_P, main = "Solubilized P concentrations per strain (Tukey)", xlab = "Strain", ylab = "Solubilized P (microg/g)", col = cp1(5), las = 2, cex = 0.8, cex.axis=0.7, cex.names=0.6)
...
As seen above, due to the big amounts of strains that have to be analyzed and compared, is much more useful the Duncan's test than the Tukey's. However, the judgment of the authors remains unclear, since the choose 5 strains according to the IAA production and only 3 from solubilized P, despite the uncertainty of this last. 

In the end, the strains chosen to be analyzed are the following: **LSM 65, 62, 68, 179, 183, 24 and 14.**


## PCA
Principal Components Analysis was done in order to see if the data groups according to some of the independent variables (crops or strains).

For this project, the package ```factoextra``` is used to carry out the PCA.
```{r General PCA, message = FALSE, warning = FALSE, include=FALSE}
...
PCA differentiated by crops:
```{r PCA crops, message = FALSE, warning = FALSE, include=FALSE}
#PCA with colored crops
groups_c <- as.factor(D_GH$Cropname)
PCAcrops <- factoextra::fviz_pca_ind(C_pca,
...
```{r Variables PCA}
PCAvariables <- factoextra::fviz_pca_biplot(C_pca,
                           col.ind = groups_c,#color by groups
                           palette = c("#00AFBB", "#E7B800", "#FC4E07", "#FF0066"),
                           legend.title = "Crops",
                           repel = TRUE)+
  labs(title= "Variables affecting the differentiation of the crops - PCA",
    caption= "Graph 12: PCA disriminating different variables as vectors that differentiate the crops.")+
...
The x axis (Dim1) separates very nicely the crops from each other, clearly differentiating the three crops from each other, based on the differences among their individuals.
...
PCA labeled according to the strains, to reveal if there is some grouping by the strains.
```{r}
PCAstrains <- factoextra::fviz_pca_ind(C_pca,geom ="point",
                         ...
                         repel = FALSE)+
  labs(title= "Strains distribution in the different groups - PCA",
    caption= "Graph 13: PCA showing how different strains are distributed among the groups.")+
  theme(plot.caption = (element_text(hjust = 0)))
PCAstrains
```

Since the differentiation is mainly on the crop type, a deeper analysis is done within each crop.

### Bean PCA
```{r PCA variable Beans, message = FALSE, warning = FALSE}
D_GHb<- D_GH[,c(3:13)]
...
```{r}
PCAbean <- factoextra::fviz_pca_var(B_pca,
                         col.var = "contrib", # Color by contributions to the PC
                         gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
                         repel = TRUE)+
  labs(title= "Variables affecting Bean - PCA",
    caption= "Graph 14: PCA disriminating different variables as vectors within the bean variables. Colours show the contribution of each variable.")+
  theme(plot.caption = (element_text(hjust = 0)))
PCAbean
```
...
PCAmaize <- factoextra::fviz_pca_var(M_pca,
                         col.var = "contrib", # Color by contributions to the PC
                         gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
                         repel = TRUE)+
  labs(title= "Variables affecting Maize - PCA",
    caption= "Graph 16: PCA disriminating different variables as vectors within the maize variables. Colours show the contribution of each variable.")+
  theme(plot.caption = (element_text(hjust = 0)))
PCAmaize
```
...
```{r}
PCAsoy <- factoextra::fviz_pca_var(S_pca,
                         col.var = "contrib", # Color by contributions to the PC
                         gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
                         repel = TRUE)+
  labs(title= "Variables affecting Soybean - PCA",
    caption= "Graph 17: PCA disriminating different variables as vectors within the soybean variables. Colours show the contribution of each variable.")+
  theme(plot.caption = (element_text(hjust = 0)))
PCAsoy
```
...
Strains are not separated even when a PCA for each crop is done. However, height and dry mass seem to be the parameters that separate the strains the most in all crops.
...
## Strain re-evaluation method

A similar process has been followed to identify the most optimal strains in the greenhouse experiments. First an ANOVA has been done, taking into account all the different assumptions. Then, for those cases in which ANOVA is not possible, the Kruskal-Wallis' test has been done. For the rest of the cases, Duncan's test was carried out to assess the different grouping options. 
...
#This function does not accept the $residuals, but since it takes the whole ANOVA result, it is assumed that the residuals are included in the code. 
...
Splot_Pshoot <- ggplot(D_Soy, aes(x=`Treatments (strains)`, y=`P Shoot g/kg`, fill=kruskalsoy8$groups[`Treatments (strains)`, "groups"])) +
  ...
  labs(title = "[P] in shoot in Soybean",
...
Important discrepancies have been found between the analysis carried out in this project and the one done by the authors: 

**Bean:** The paper mentioned there isn't any significance difference in P levels in Roots. We found otherwise.

**Soy:** The original work mentioned that there is significance difference in root and total dry mass, however our analysis showed no significant difference.

Moreover, the paper mentioned no significant difference in P levels, when in fact there has been.

Also, the use of barplots in order to display the graphs is unadvised. Instead, boxplots are recommended to show the performance of each strain in each parameter. This plots are better for displaying quantitative data, because they deal with medians, quartiles, and information related to actual quantities. Barplots are used for displaying categorical/qualitative data because they deal with counts.


```{r SUmmary Table}
newvar.name.bean <- c("Plant Height", "Shoot dry mass", "Root dry mass", "Total dry mass", "P soil", "P shoot", "P root", "N shoot", "N root")
newstrain.bean <-  c(strain.bean)
newstrain.maize <- c(strain.maize)
newstrain.soy <-  c(strain.soy)

newtable <- data.frame(newvar.name.bean, newstrain.bean, newstrain.maize, newstrain.soy) 

newtable<- newtable%>% 
  rename("Variable" = newvar.name.bean,
         "Bean"= newstrain.bean,
         "Maize"= newstrain.maize,
         "Soybean" = newstrain.soy)
kbl(newtable)%>%
  kable_styling(full_width = F) %>% 
  footnote(general = "Table 5: Summary table of all the optimal strains of the three different crops.", general_title = "")
...
**Bean:** Significant differences in P levels in Shoot and in Root however in parameters like the plant height in P levels in soil, the control was better than any of the strains.

**Maize:** Significant differences in almost all parameters.

**Soybean:** Significant differences in plant height, & P levels in root.

### Interms of strains

Strains 24 and 65 seem to have the highest effect on Maize crops.

As for Bean and Soybean strains 183, 179 and 14 have the highest effect (only on a couple of parameters)

### In terms of parameters: 

No strains showed any positive change on the parameter of the Root dry mass, if anything strains reduced the root dry mass in the Maize crop.
