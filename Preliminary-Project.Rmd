...
3. Carry out a Principal Components Analysis (PCA) and a Linear Discriminant Analysis (LDA) in order to address the possible differences in a driven analysis where the groups are determined previously. 
...
## PCA
Principal Components Analysis was done in order to test if in a driven analysis, where the different groups are defined by the researcher previously according to a specific criterion, in this case the different crops and strains, differences in the performance can be observed. 
...
PCAcrops <- factoextra::fviz_pca_ind(C_pca,
             ...
             legend.title = "Crops",
             repel = TRUE)+
  labs(caption= "Graph 12: PCA on the individuals, with the crops shown in different colors.")+
...
PCAvariables <- factoextra::fviz_pca_biplot(C_pca,
  ...
  labs(title= "Variables affecting the differentiation of the crops - PCA",
    caption= "Graph 13: PCA disriminating different variables as vectors that differentiate the crops.")+
  theme(plot.caption = (element_text(hjust = 0)))
PCAvariables
...
PCA labbed according to the strains, to reveal if there is some grouping by the strains.
```{r}
...
PCAstrains <- factoextra::fviz_pca_ind(C_pca,geom ="point",
  ...
  labs(title= "Strains distribution in the different groups - PCA",
    caption= "Graph 14: PCA showing how different strains are distributed among the groups.")+
  theme(plot.caption = (element_text(hjust = 0)))
PCAstrains
```

Since a differentiation of the strain is not clear considering all the crops at once, a PCA for every crop will be made.

### Bean PCA
```{r PCA variable Beans, message = FALSE, warning = FALSE, include=FALSE}
D_GHb<- D_GH[,c(3:13)]
...
PCAbean <- factoextra::fviz_pca_var(B_pca,
  ...
  labs(title= "Variables affecting Bean - PCA",
    caption= "Graph 15: PCA disriminating different variables as vectors within the bean variables. Colours show the contribution of each variable.")+
  theme(plot.caption = (element_text(hjust = 0)))
PCAbean
```
