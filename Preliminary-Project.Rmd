...
In this project, we re-analyze the results obtained by Baron et al. in their article [" Purpureocillium lilacinum and Metarhizium marquandii as plant growth-promoting fungi."](PeerJ 8:e9005 DOI 10.7717/peerj.9005) using RStudio. The authors test the performances of different fungi obtained from a soil sample while being used as plant growth-promoting organisms. 

First, their IAA and P solubilization is tested in order to address the more appropriate strains to be tested in green house experiment. 

After that, the second part consists in the test of plan-growth related parameters in presence of the strains chosen in the first part. 
...
The F test only compares two variables at a time, an Analisis of Variance (ANOVA) was done instead for both datasets
...
The Idol Acetic Acid (IAA) produced and the Phosphorous solubilized by each strain was tested in order to choose the ones with the highest amounts. In the article an F and Tukey tests were done to figure this out. Nevertheless, although the IAA data was homeoscedastic, when the residuals of the IAA concentrations were tested for normality, it was found that their residues do not follow a normal distribution and thus the F and Tukey tests could not be conducted if a data transformation process wasn't made first.
...
```
We chose to do the Lillie test because...
...
plot(T_nIAA)
```
The analysis results in 5 strains being in the first and second groups, the (65,62,68,179,183)



They chose strains belonging to the groups A, B and C (65,62,68,179,183)
...
As the residuals were normal and the differences between strains were significant, a Tukey test was conducted.
...
```
In out tukey test, the strains were only separated into three groups, only strain 24 was completely separated from the other strains with the higher concentrations. So with our analysis we can not justify statistically, choosing the first 3 strains with higher concentrations.
...
They chose the A and AB groups (strains 24,65 and 14)
...
### Green-House infection experiments

From this analysis we got the next conclusions: 

1) We have obtained the same results as in the article.
2) Duncan and Tukey post-hoc analyses present the similar results with this data. However bibliography shows that Tukey is more conservative: it will make bigger groups, asuming that there aren't that many differences between groups. 
...
var.name.maize <- c("Plant Height", "Plant Height", "Shoot dry mass","Shoot dry mass", "Root dry mass", "Root dry mass", "Total dry mass", "Total dry mass", "P soil", "P shoot", "P root","N shoot", "N shoot", "N root", "N root" )
...
var.name.soy <- c("Plant Height", "Plant Height", "Shoot dry mass","Shoot dry mass", "Root dry mass", "Root dry mass", "Total dry mass", "Total dry mass", "P soil", "P shoot", "P root","N shoot", "N shoot", "N root", "N root" )
...
**Bean** it's mostly affected by strain LSM 183 and 24 in the Shoot dry mass, LSM 183 in the total dry mass, LSM 183, 68 and 179 in the content of P in the shoot.

**Soybean** is mainly affected by strain LSM 179 in the Plant Height, LSM 14, 24 and 62 in the P shoot and  LSM 183 and 179 in the P root.
