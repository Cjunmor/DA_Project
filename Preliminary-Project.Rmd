...
# kr$groups
# plot (kr)
D_IAA$names <- paste0("LSM ", D_IAA$LSM_strain ) #Add column with strain name
kr_p <- ggplot(D_IAA, aes(x=names, y=IAA_pg, fill=kr$groups[LSM_strain, "groups"])) +
  geom_boxplot()+
  labs(fill = "Strain groups") +
  xlab('Strain') +
  ylab('Indolacetic acid produced (microg/g)')+
  labs(title = "Indolacetic acid produced by each strain (Non Parametric Analysis)",
       caption= "Graph 1: Indolacetic acid concentrations Holms groups (microgram per gram of dried micelium)")+
...
n_D_IAA$names <- paste0("LSM ", n_D_IAA$D_IAA.LSM_strain ) #Add column with strain name


Tn_p <- ggplot(n_D_IAA, aes(x=names, y=n_IAA, fill=T_nIAA$groups[D_IAA.LSM_strain, "groups"])) +
  geom_boxplot()+
  labs(fill = "Strain groups") +
  xlab('Strain') +
  ylab('Indolacetic acid produced (microg/g)')+
  labs(title = "Indolacetic acid produced by each strain (Parametric Analysis)",
       caption= "Graph 2: Indolacetic acid concentrations Tukey groups (microgram per gram of dried micelium)")+
  theme(plot.title = element_text(hjust = 0.5),
        plot.caption = element_text(hjust = 0), 
        axis.text.x = element_text(angle = 90))
Tn_p
...
Dn_p <- ggplot(n_D_IAA, aes(x=names, y=n_IAA, fill=D_nIAA$groups[D_IAA.LSM_strain, "groups"])) +
  geom_boxplot()+
  labs(fill = "Strain groups") +
  xlab('Strain') +
  ylab('Indolacetic acid produced (microg/g)')+
  labs(title = "Indolacetic acid produced by each strain (Parametric Analysis)",
       caption= "Graph 3: Indolacetic acid concentrations Duncan groups (microgram per gram of dried micelium)")+
  theme(plot.title = element_text(hjust = 0.5),
        plot.caption = element_text(hjust = 0), 
        axis.text.x = element_text(angle = 90))
Dn_p
...
# D_Phosphorus$names <- paste0("LSM ", D_Phosphorus$LSM_strain ) #Add column with strain name
# 
# TP_p <- ggplot(D_Phosphorus, aes(x=names, y=P_pg, fill=T_P$groups[LSM_strain, "groups"])) +
#   geom_boxplot()+
#   labs(fill = "Strain groups") +
#   xlab('Strain') +
#   ylab('Phosphorus solubilized (microg/g)')+
#   labs(title = "Phosphorus solubilized by each strain (Parametric Analysis)",
#        caption= "Graph 3: Solubilize phosphorus concentrations, Duncan groups (microgram per gram of dried micelium)")+
#   theme(plot.title = element_text(hjust = 0.5),
#         plot.caption = element_text(hjust = 0),
#         axis.text.x = element_text(angle = 90))
# TP_p
...
plot.new()
plot(T_P, main = "Solubilized P concentrations per strain (Duncan)", xlab = "Strain", ylab = "Solubilized P (microg/g)", col = cp1(3), las = 2, cex = 1.1)
