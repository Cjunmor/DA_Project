...
strain.bean <-  c("Control","LSM_183/LSM_24", "None","None","None", "LSM_183", "Control", "LSM_183/LSM_68/LSM_179","LSM_14", "None", "LSM_14", "None", "None")
posthoc.bean <- c("Kruskal-Wallis", "Kruskal-Wallis","Tukey", "Duncan","Tukey", "Duncan", "Kruskal-Wallis","Kruskal-Wallis","Kruskal-Wallis","Tukey", "Duncan","Tukey", "Duncan")
var.name.bean <- c("Plant Height", "Shoot dry mass", "Root dry mass","Root dry mass", "Total dry mass","Total dry mass", "P soil", "P shoot", "P root", "N shoot", "N shoot", "N root", "N root")
bean.name <- c(rep("Bean", 13))

data.strains.beans <- data.frame(bean.name, var.name.bean, posthoc.bean, strain.bean)
...
kbl(data.strains.beans) %>% 
  ...
  footnote(general = "Optimal strains for each variable measured in bean greenhouse experiments adressed by Kruskal-Wallis, Tukey or Duncan analysis.", general_title = "")

#Maize
strain.maize <-  c("LSM_65","LSM_65","LSM_65","LSM_65","Control", "Control", "Control/LSM_65","Control/LSM_65", "LSM_24", "LSM_24", "Control/LSM_65/LSM_14/LSM_24/LSM_62", "None", "None","LSM_65/LSM_24","LSM_65/LSM_24")
posthoc.maize <-  c("Tukey", "Duncan","Tukey", "Duncan","Tukey", "Duncan","Tukey", "Duncan","Kruskal-Wallis","Kruskal-Wallis","Kruskal-Wallis","Tukey", "Duncan","Tukey", "Duncan" )
var.name.maize <- c("Plant Height", "Plant Height", "Shoot dry mass","Root dry mass", "Root dry mass", "Shoot dry mass", "Total dry mass", "Total dry mass", "P soil", "P shoot", "P root","N shoot", "N shoot", "N root", "N root" )
maize.name <-  c(rep("Maize", 15))

data.strains.maize <-  data.frame(maize.name, var.name.maize, posthoc.maize, strain.maize)
...
kbl(data.strains.maize) %>% 
  ...
  column_spec(c(1:4), border_left = FALSE, border_right = FALSE) %>% 
  collapse_rows(columns = 1:2, valign = "middle") %>% 
  footnote(general = "Optimal strains for each variable measured in maize greenhouse experiments adressed by Kruskal-Wallis, Tukey or Duncan analysis.", general_title = "")

#Soy
strain.soy <- c("LSM_179","LSM_179", "None","None","None", "LSM_24", "None", "LSM_24", "None", "LSM_14/LSM_24/LSM_62", "LSM_183/LSM_179","None", "None","None","None")
posthoc.soy <- c("Tukey", "Duncan","Tukey", "Duncan","Tukey", "Duncan","Tukey", "Duncan","Kruskal-Wallis","Kruskal-Wallis","Kruskal-Wallis","Tukey", "Duncan","Tukey", "Duncan" )
var.name.soy <- c("Plant Height", "Plant Height", "Shoot dry mass","Root dry mass", "Root dry mass", "Shoot dry mass", "Total dry mass", "Total dry mass", "P soil", "P shoot", "P root","N shoot", "N shoot", "N root", "N root" )
soy.name <-  c(rep("Soybean", 15))

data.strains.soy <-  data.frame(soy.name, var.name.soy, posthoc.soy, strain.soy)
...
kbl(data.strains.soy) %>% 
  ...
  column_spec(c(1:4), border_left = FALSE, border_right = FALSE) %>% 
  collapse_rows(columns = 1:2, valign = "middle") %>% 
  footnote(general = "Optimal strains for each variable measured in soybean greenhouse experiments adressed by Kruskal-Wallis, Tukey or Duncan analysis.", general_title = "")
